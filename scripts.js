...
function escribirMensajeAsunto(mensaje) {
  var capturar = document.querySelector(".error__asunto");
  capturar.innerHTML = mensaje;
  capturar.style.visibility = "visible";
}
...
function validarAsunto () {
  let capturarElemento = document.querySelector("#asunto");
  let contenido = capturarElemento.value;
  let letras = contenido.split('');
  var error = document.querySelector(".error__asunto");
    if (letras.length==0) {
      escribirMensajeAsunto("Por favor, escriba un asunto.");
    }
    else if (letras.length<=50){
      error.style.visibility = "hidden";
    }
    else if (letras.length>50) {
      escribirMensajeAsunto("Por favor, este campo no debe superar los 50 caracteres.");
    }                              
}
